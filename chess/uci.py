...
class Engine(object):
    def __init__(self, process, chess_db=False):
        ...
        *depth*, *nodes*, *mate*, *infinite* or some time control settings,
        ...
        :return: **In normal search mode** a tuple of two elements. The first
            ...
            **ponder mode** there is no result. See *stop* (or *ponderhit*)
            instead.
        """
        result = self._queue_command(GoCommand(searchmoves, ponder, wtime, btime, winc, binc, movestogo, depth, nodes, mate, movetime, infinite), async_callback)
        return result
